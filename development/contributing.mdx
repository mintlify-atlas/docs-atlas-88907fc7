---
title: Contributing to BOOM
description: Learn how to contribute to BOOM, including error handling conventions, instrumentation practices, and logging guidelines
---

## Overview

We welcome contributions to BOOM! This guide covers our development conventions and best practices for error handling, instrumentation, and logging.

## Errors, instrumentation, and logging

Here are some conventions we try to adhere to. It's important to emphasize that none of this is set in stone. We evolve as our needs change and as we discover better ways of doing things.
...
<Warning>
Panicking should be reserved for truly unrecoverable errors, such as things that go wrong during startup.
</Warning>
...
We use `tracing::Span`s as the foundation for instrumenting BOOM. Briefly, a span represents a duration, e.g., the execution of a function. Each span has a name and optionally a set of values recorded as fields. Spans can be nested.
...
The `tracing::instrument` attribute is a preferred way to define a span for a function or method. This is not the only way to define a span, and spans are not required to be tied to functions, but using `instrument` is very convenient.

```rust
#[instrument(skip(self))]
fn my_method(&self, arg: String) {
    // function body
}
```
...
As for deciding *what* to instrument, we should consider instrumenting:

1. Anything that helps provide context for events (particularly ERROR events)
2. Anything that we're interested in profiling for performance
...
Strive to log things at the appropriate level:
...
<Accordion title="DEBUG and TRACE levels">
...
- The `log_error` and `as_error` macros help standardize how we report errors as ERROR events. See the `o11y` module for details
- Some errors occur more than once in a given function. `log_error`/`as_error` can be used to give each occurrence a unique event callsite and therefore a clear origin when logged

```rust
foo()
    .inspect_err(as_error!("an optional message for additional context"))?;
```
...
<Steps>
  <Step title="Install inferno">
    Make sure you have `inferno` installed:
  ...
  <Step title="Run BOOM with flame graph generation">
    Run BOOM with the `BOOM_FLAME_FILE` environment variable. This instructs BOOM to generate a flame graph and save the output at the given path:
  ...
  <Step title="Generate the SVG">
    Turn the raw output into an interactive SVG with `inferno-flamegraph`:
...
We rely on [GitHub issues](https://github.com/boom-astro/boom/issues) to track bugs and feature requests. Before starting work on a major feature, please open an issue to discuss your approach.
